...
impl Field {
    fn move_direction(&self, direction: &Direction, position: (usize, usize)) -> Option<(usize, usize)> {
        let (x, y) = position;
        match direction {
            Direction::North => if y > 0 { Some((x, y - 1)) } else { None },
            Direction::East => if x + 1 < self.columns { Some((x + 1, y)) } else { None },
            Direction::South => if y + 1 < self.rows { Some((x, y + 1)) } else { None },
            Direction::West => if x > 0 { Some((x - 1, y)) } else { None },
        }
    }
    ...
    fn follow_trail(&self, previous: (usize, usize), position: (usize, usize), direction: &Direction, distance: u32) -> Option<u32> {
        let (x, y) = position;
        ...
        if !self.get_tile(previous).connects(&direction) || !self.get_tile(position).connects(&direction.opposite()) {
            return None;
        }

        println!("{} {} {} {}", self.get_tile(previous), direction, self.get_tile(position), distance);

        let valid_directions = DIRECTIONS.iter().filter(|d| *d != &direction.opposite()).map(|d| (d, self.move_direction(d, position))).filter_map(|(d, p)| p.map(|pos| (d, pos)));
        return valid_directions.filter_map(|(new_direction, new_position)| self.follow_trail(position, new_position, &new_direction, distance + 1)).max()
    }
}
...
fn main() {
    ...
    let mut field = Field { columns, rows, tiles: vec![vec![Tile::Ground; rows]; columns] };
    ...
    let valid_directions = DIRECTIONS.iter().map(|d| (d, field.move_direction(d, start_position))).filter_map(|(d, p)| p.map(|pos| (d, pos)));
    let distance = valid_directions.filter_map(|(new_direction, new_position)| field.follow_trail(start_position, new_position, &new_direction, 0)).max().unwrap();
